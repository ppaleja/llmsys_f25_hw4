...
# [Assignment 3: Transformer Architecture - CMU 11868 LLM Systems 2025 Fall Assignments](https://llmsystem.github.io/llmsystemhomework/assignment_3/)
...
[https://github.com/llmsystem/llmsys\_f25\_hw3](https://github.com/llmsystem/llmsys_f25_hw3)
...
If you encounter an **"Aborted (core dumped)"** error while running PyTorch, it is likely due to an **incompatibility between `nvcc` and the NVIDIA driver version** used by PyTorch. This happens when: - `nvcc` (the CUDA compiler) is **newer than** the supported CUDA version in the NVIDIA driver. - PyTorch is built for a different CUDA version than the one installed.
...
Refer to [[llmsys-04-autodiff-2ff210695355c7f9089b6209c515731c.pdf#page=6&rect=36,120,824,523|llmsys-04-autodiff-2ff210695355c7f9089b6209c515731c, p.6|slide 6 here]] for more details.
...
After correctly implementing the functions, you should be able to pass tests marked as logsumexp and softmax\_loss by running:
...
1.  Projecting \\(X\\) into \\(Q, K^T, V\\) in the `project_to_query_key_value` function.
    ...
    -   \\(Q \\in R^{B×S×(h×D\_h)}\\) gets ==unraveled==[^1] to \\(Q \\in R^{B×S×h×D\_h}\\)
    -   \\(Q \\in R^{B×S×h×D\_h}\\) gets ==permuted==[^2] to \\(Q \\in R^{B×h×S×D\_h}\\)
...
2.  Computing Self-Attention
    ...
    Let \\(Q\_i\\), \\(K\_i\\), \\(V\_i\\) be the Queries, Keys, and Values for head \\(i\\). You'll need to compute: $$\text{softmax}\left(\dfrac{Q_iK_i^T}{\sqrt{D_h}} + M\right)V_i$$ with batched matrix multiplication (which we've implemented for you) across each batch and head. \\(M\\) is the causal mask added to prevent your transformer from attending to positions in the future, which is crucial in an auto-regressive language model.
...
![Transformer Layer Normalization](ReadItLater%20Inbox/assets/Assignment%203%20Transformer%20Architecture%20-%20CMU%2011868%20LLM%20Systems%202025%20Fall%20Assignments-Cn7EPDIJrg.png)  
...
![Decoder Transformer](ReadItLater%20Inbox/assets/Assignment%203%20Transformer%20Architecture%20-%20CMU%2011868%20LLM%20Systems%202025%20Fall%20Assignments-Tc4HF0na9P.png)

*(Image from [Transformer Decoder](https://arxiv.org/pdf/1706.03762.pdf))*

Combine all components to create the final model. Given an input tensor \\(X\\) with shape \\((\\text{batch size}, \\text{sequence length})\\):
...
![Performance Chart](ReadItLater%20Inbox/assets/Assignment%203%20Transformer%20Architecture%20-%20CMU%2011868%20LLM%20Systems%202025%20Fall%20Assignments-E3YL2l1JJ4.png)
...
[^1]: Unraveled in this context (as well as permute) use `view` to reshape the tensor.

[^2]: Permute in this context means use `transpose` to permute the dimensions of the tensor.
